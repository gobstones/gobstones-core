 ...
 * line arguments, etc. The type {@link SourceInput} establishes the different
 * kinds of input a {@link SourceReader} accepts to read, independently of how
 * it was obtained.
 ...
 * tuple index-line-column. That is, the main problem it solves is that of
 * calculating the position of each character read by taking into account
 * characters indicating the end-of-line. It also solves the problem of input
 * divided among several documents, as it is usually the case with source code,
 * and it provides a couple of additional features:
 ...
 * {@link AbstractSourcePosition}s, in particular
 * {@link AbstractKnownSourcePosition}s, can be read from it. Possible
 * interactions with a {@link SourceReader} include:
 ...
 *  - get the current position as a {@link AbstractKnownSourcePosition}, with
 *    {@link SourceReader.getPosition},
 *  - get the current position as a {@link AbstractDocumentSourcePosition}, with
 *    {@link SourceReader.getDocumentPosition}, provided the end of input was
 *    not reached,
 *  - detect if the end of input was reached, with
 *    {@link SourceReader.atEndOfInput},
 ...
 *    {@link takeWhile}, and
 *  - manipulate "regions", with {@link SourceReader.beginRegion} and
 *    {@link SourceReader.endRegion}. When reading from sources with multiple
 *    documents of input, skipping moves inside a document until there are no
 *    more characters, then an end of document position is reached (a special
 *    position just after the last character of that document), and then a new
 *    document is started. Regions are reset at the beginning of each document.
 *    When the last document has been processed, and the last end of document
 *    has been skipped, the end of input is reached (a special position just
 ...
 * Visible characters affect the line and column calculation, and, conversely,
 * non visible characters do not. Characters are marked as visible by skipping
 * over them normally; characters are marked as non visible by silently skip
 * over them. Visibility of the input affect the information that positions may
 * provide. When skipping characters, at the end of each of the input document
 * there is a special position that must be skipped, but that has no character,
 * and thus, cannot be peeked -- the {@link EndOfDocumentSourcePosition}. This
 * position cannot be skipped as non visible, as every input document is known
 * by the user.
 *
 * Regarding regions, a "region" is some part of the input that has an ID (as a
 * string). It is used in handling automatically generated code. A typical use
 * is to identify parts of code generated by some external tool, in such a way
 * as to link that part with the element generating it through region IDs.
 * Regions are supposed to be nested, so a stack is used, but no check is made
 * on their balance, being the user responsible for the correct pushing and
 * popping of regions. When skipping moves from one source document to the next,
 * regions are reset, as regions are not supposed to cross different documents
 * of the input.
 *
 *  **Example**
 *
 * This is a very basic example using all basic operations. A more complex
 * program will use functions to organize the access with a logical structure,
 * and it will also consider different inputs in the source. Just use this
 * example to understand the behavior of operations -- common usage do NOT
 ...
 * ```
 *
 * NOTE: as {@link SourceReader.peek} is partial, not working at the end of
 *       documents, each of its uses must be done after confirming that
 *       {@link SourceReader.atEndOfDocument} is false. For that reason document
 *       is better to use {@link SourceReader.startsWith} to verify if the input
 *       starts with some character (or string), when peeking for something
 *       specific.
 * @group API: Main
 */
export class SourceReader {
     ...
     * converted to `Record<string, string>` for ease of access. The
     * {@link SourceReader._charIndex | _charIndex } either points to a valid
     ...
     * {@link SourceReader.skip | skip}. When changing from one document to the
     * next, line and column numbers are reset.
     *
     ...
     *
     * This class is tightly coupled with {@link AbstractSourcePosition} and its
     * subclasses, because of instances of that class represent different
     * positions in the source inputs kept by a {@link SourceReader}. The
     * operations {@link SourceReader._documentNameAt | _documentNameAt},
     ...
     * and {@link SourceReader._fullInputFromTo | _fullInputFromTo}, and the
     * static value {@link SourceReader.defaultDocumentNamePrefix | _unnamedDocument} are
     * meant to be used only by {@link AbstractSourcePosition}, to complete
     * their operations, and so they are grouped as Protected.
     *
     * The remaining auxiliary operations are meant for internal usage, to
     * provide readability or to avoid code duplication. The auxiliary operation
     * {@link SourceReader._cloneRegions | _cloneRegions } is needed because
     * each new position produced with
     ...
     */
    public static UnknownPosition: SourcePosition = SourcePositions.Unknown();
    // -----------------------------------------------
    ...
    // -----------------------------------------------
    /**
     * The string to use as a name for unnamed input documents. It is intended
     * to be used only by instances and {@link AbstractSourcePosition}.
     *
     * @group API: Static Properties
     */
    public static defaultDocumentNamePrefix: string = 'doc';
    // -----------------------------------------------
    ...
    /**
     * The characters used to indicate the end of a line. These characters
     * affect the calculation of line and column numbers for positions.
     *
     * @group API: Properties
     */
    ...
    /**
     * The current input index. The current input is that in
     * `_documents[_documentsNames[_documentIndex]]` when `_documentIndex <
     * _documentsNames.length`.
     *
     ...
     *
     * **INVARIANT:** if `_documentIndex < _documentsNames.length` then `0 <=
     *  _charIndex < _documents[_documentsNames[_documentIndex]].length`
     * @group Implementation: Internal state
     * @private
     */
    ...
    /**
     * A copy of the visible parts of the input documents. A part is visible if
     * it has been skipped, and that skip was not silent (see
     ...
     * **INVARIANTS:**
     *   * `0 <= _line`
     *   * if `_documentIndex < _documentsNames.length` then `_line <
     *      _documents[_documentsNames[_documentIndex]].length`
     * @group Implementation: Internal state
     * @private
     */
     ...
     * **INVARIANTS:**
     *   * `0 <= _column`<
     *   * if `_documentIndex < _documentsNames.length`, then `_column <
     *      _documents[_documentsNames[_documentIndex]].length`
     * @group Implementation: Internal state
     * @private
     */
    ...
    // -----------------------------------------------
    /**
     * A new {@link SourceReader} is created from the given `input`. It starts
     * in the first position of the first input document (if it is empty, starts
     * in the end of document position of that document). Line enders must be
     * provided, affecting the calculation of line and column for positions. If
     * there are no line enders, all documents in the source input are assumed
     ...
     * the end of a line.
     * @throws {@link ErrorNoInput} if the arguments are undefined or has no
     * documents.
     * @group API: Creation
     */
    public constructor(input: SourceInput, lineEnders: string = '\n') {
        ...
        if (typeof input === 'string') {
            ...
            // will be referred as "doc" afterwards.
    ...
    /**
     * Answers if there are no more characters to read from the current
     * document.
     * @group API: Access
     */
    ...
    /**
     * Gives the current char of the current input document. See
     * {@link SourceReader} for an example.
     *
     * **PRECONDITION:** `!this.atEndOfInput() && !this.atEndOfDocument`
     * @throws {@link ErrorAtEndOfInputBy} if the source reader is at EndOfInput
     *         in the current position.
     * @throws {@link ErrorAtEndOfDocumentBy} if the source reader is at
     *         EndOfDocument in the current position.
     * @group API: Access
     */
    public peek(): string {
     ...
     * documents -- that is, only the current input document is checked. See
     * {@link SourceReader} documentation for an example.
     *
     * @param str The string to verify the current input, starting at the
     * current char.
     *
     * @group API: Access
     */
    ...
    /**
     * Gives the current position as a {@link AbstractKnownSourcePosition}. See
     * {@link SourceReader} documentation for an example.
     *
     * NOTE: the special positions at the end of each input document, and at the
     *       end of the input can be accessed by
     *       {@link SourceReader.getPosition}, but they cannot be peeked.
     * @group API: Access
     */
    ...
    // -----------------------------------------------
    /**
     * Skips the given number of chars in the input, moving forward. It may skip
     * documents, considering the end of document as a 'virtual' char.
     *
     * If the argument is a string, only its length is used (i.e. its contents
     * are ignored). Negative numbers do not skip (are equivalent to 0). At the
     * end of each input document, an additional skip is needed to start the
     * next input document. This behavior allows the user to be aware of the
     * ending of documents. Regions are reset at the end of each documents (the
     ...
     * is visible, and thus it is added to the visible inputs. The end of each
     * input document cannot be skipped silently, and thus for that particular
     ...
     * returning the characters skipped. All contiguous characters from the
     * initial position satisfying the predicate are read. It guarantees that
     * the first character after skipping, if it exists, does not satisfy the
     * predicate. It does not go beyond the end of the current document, if
     * starting inside one.
     * @param contCondition A predicate on strings, indicating the chars to
     * read.
     ...
     *                 visible input.
     * @result The string read from the initial position until the character
     *         that do not satisfy the condition or the end of the current
     *         string.
     * @group API: Modification
     */
    ...
    /**
     * Pushes a region in the stack of regions. It does not work at the
     * EndOfInput or the EndOfDocument (it does nothing).
     * @group API: Modification
     */
    ...
    /**
     * Pops a region from the stack of regions. It does nothing if there are no
     * regions in the stack.
     * @group API: Modification
     */
    ...
    // -----------------------------------------------
    /**
     * Gives the name of the input document at the given index. It is intended
     * to be used only by {@link AbstractSourcePosition}.
     *
     * **PRECONDITION:** `index <= this._documentsNames.length` (not verified)
     * As it is a protected operation, it is not expectable to receive invalid
     * indexes. It is not taken into account which are the results if that
     * happens.
     ...
     * and non-visible. It is intended to be used only by
     * {@link AbstractSourcePosition}.
     *
     * **PRECONDITION:** `index < this._documentsNames.length` (not verified)
     *
     * As it is a protected operation, it is not expectable to receive invalid
     ...
     * is intended to be used only by {@link AbstractSourcePosition}.
     *
     * PRECONDITION: `index < this._documentsNames.length` (not verified).
     *
     * As it is a protected operation, it is not expectable to receive invalid
    ...
    /**
     * Gives the contents of the input between two positions. If `from` is not
     * before `to`, the result is the empty string. It is intended to be used
     * only by {@link AbstractSourcePosition}.
     *
     * **PRECONDITION:** both positions correspond to this reader (and so are >=
     * 0 -- not verified)
     * @group Implementation: Protected for Source Positions
     * @private
     */
    // public _fullInputFromTo(from: SourcePosition, to: SourcePosition): string {
    //    return this._inputFromToIn(from, to, false);
    // }

    /**
     * Gives the contents of the visible input between two positions. If `from`
     * is not before `to`, the result is the empty string. It is intended to be
     * used only by {@link AbstractKnownSourcePosition} subclasses.
     *
     * **PRECONDITION:** both positions correspond to this reader (and so are >=
     * 0 -- not verified)
     * @group Implementation: Protected for Source Positions
     * @private
     */
    // public _visibleInputFromTo(from: SourcePosition, to: SourcePosition): string {
    //    return this._inputFromToIn(from, to, true);
    // }

    /**
     * Returns the document context of the requested document before the
     * requested character index, up to the requested character index,
     * divided in lines.
     *
     * If the requested number of lines is 0, only the line that the
     * character index belongs to is returned.
     *
     * If the requested lines is greater than 0, then as many lines before
     * the one containing the character index are returned. If there are less
     * lines than the amount requested, then all the lines are returned
     * up to the one containing the character index.
     *
     * The line containing the character index is always returned,
     * from the start, up to the character index itself (NOT including the
    ...
    // -----------------------------------------------
    /**
     * Skips the input positioning the reader at the start
    ...
    /**
     * Gives the contents of either the full or visible input between two
     * positions, depending on the `visible` argument. If `from` is not before
     * `to`, the result is the empty string.
     *
     * **PRECONDITIONS:**
     *  * both positions correspond to this reader (and so are >= 0 -- not
     *    verified)
     * @group Implementation: Auxiliaries
     * @private
     */
    ...
    /**
     * Gives a clone of the stack of regions. Auxiliary for
     * {@link SourceReader.getPosition | getPosition}. It is necessary because
     * regions of {@link AbstractSourcePosition} must correspond to those at
     * that position and do not change with changes in reader state.
     * @group Implementation: Auxiliaries
     * @private
     */
