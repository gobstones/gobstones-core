...
import { Direction } from '../../src/gobstones-lang';
...
describe(`A Direction should`, () => {
    ...
    it(`Answers it's value correctly`, () => {
        expect(Direction.North.value).toBe(Direction.NORTH);
        expect(Direction.East.value).toBe(Direction.EAST);
        expect(Direction.South.value).toBe(Direction.SOUTH);
        expect(Direction.West.value).toBe(Direction.WEST);
    });
});
